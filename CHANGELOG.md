...
## [Unreleased]
### Fixed
- **Docker shim: `POST /auth` (docker login) implemented.** The route didn't exist, so any client logging in through the compat socket (the docker CLI, Komodo periphery's registry login) hit axum's bare empty 404 and surfaced it as a JSON parse error. The new handler validates the submitted credentials against the registry itself (v2 ping → `WWW-Authenticate` Bearer-token or Basic challenge; Docker Hub aliases collapse to `registry-1.docker.io`; loopback registries get a plain-HTTP fallback) and answers dockerd's wire shapes (`200 {"Status":"Login Succeeded"}` / `401 {"message":…}`). Valid credentials are also persisted into the daemon's registry credential store (replace-then-create, so re-login rotates the secret) so native zlayer pulls use them too. (`crates/zlayer-docker/src/socket/registry_login.rs`)
- **Docker shim: stdcopy-framed log bodies are now labelled `application/vnd.docker.multiplexed-stream`** (dockerd's API ≥ 1.42 contract) instead of `vnd.docker.raw-stream`, on `/containers/{id}/logs`, `/services/{id}/logs`, and `/tasks/{id}/logs`, plus the daemon's internal `format=raw` endpoint. Clients that pick their decoder off the Content-Type (bollard — Komodo periphery's docker client) were treating the multiplexed body as raw bytes and leaking the 8-byte frame headers (`\x01…`) into the text — Komodo's "failed to parse container logs, invalid char 1". (`crates/zlayer-docker/src/socket/containers.rs`, `crates/zlayer-docker/src/socket/swarm/{services,tasks}.rs`, `crates/zlayer-api/src/handlers/containers.rs`)
- **Docker shim: unknown routes return dockerd's JSON 404** (`{"message":"page not found"}`) instead of axum's empty-body 404, so Docker SDKs report a clean not-found instead of a decode error on every endpoint the shim doesn't implement. (`crates/zlayer-docker/src/socket/mod.rs`)
- **Release artifact downloads resume instead of restarting.** The release workflow's curl steps used fixed `--max-time` restarts-from-zero; against a slow forge the 46 MB WSL2 rootfs burned 4×600 s and never finished. All artifact downloads now go through the new `Public/actions/download-resumable` composite action (byte-offset resume via `curl -C -`, stall detection via `--speed-limit/--speed-time`); the GitHub release workflow gets the same loop inline (a public repo can't `uses:` the private actions mirror). (`.forgejo/workflows/release.yml`, `.github/workflows/release.yml`)
- **macOS release binaries are signed with the virtualization entitlement in CI, and `install.sh` self-heals unsigned ones.** An installed `zlayer` without `com.apple.security.virtualization` cannot boot any VZ guest, which left `zlayer build` dead on macOS (`zlayer-buildd` stuck `0/1 pending`). Both macOS build jobs now sign via `scripts/sign-vz.sh` using the Mac runner's keychain identity (Developer ID preferred, first valid identity next, ad-hoc fallback), and `install.sh` re-signs ad-hoc at install time whenever the entitlement is missing or the existing signature is only ad-hoc — never downgrading a Developer ID signature. (`.forgejo/workflows/build.yml`, `install.sh`)
- **`zlayer daemon install --docker-socket` no longer installs `docker`/`docker-compose` CLI shims.** The flag enables only the Docker-compatible socket; the unconditional shim install (all three platforms) could silently overwrite a real Docker CLI (Docker Desktop on macOS) or land off-PATH in `~/.zlayer/bin`. Shim installation is exclusively `zlayer docker install`'s job, which has explicit `--no-shim` semantics. Uninstall still cleans up any stale shims. (`bin/zlayer/src/commands/daemon.rs`)
- **`zlayer build` on macOS hard-fails fast on the two buildd guest preconditions instead of failing cryptically mid-build.** `ensure_buildd` now syncs the buildd guest clock from the host before every build (`date -u -s` + read-back skew check; a stale VZ guest clock made freshly-minted mTLS certs "not yet valid", which is also why a healthy buildd could never pass its 60s health gate) and verifies `/dev/fuse` + `fuse-overlayfs` with retries, erroring with the active image name instead of letting buildah die with `fuse-overlayfs: exit status 1`. (`bin/zlayer/src/commands/buildd_manager.rs`)
- **`install-dev.sh --replace` prints its mode and gates the `/var/run/docker.sock` takeover on actual daemon readiness** (30×1s `status` poll with the right log path on timeout) instead of a blind `sleep 3`. (`scripts/install-dev.sh`)
- **Linux release tarballs now include `zlayer-overlayd`, and `install.sh` stages it next to the `zlayer` binary.** Previously the overlay network daemon was never shipped in any release artifact, so fresh installs lacked it and any flow that registers/starts the overlay service (`zlayer daemon install`, hub installs) broke because the main daemon couldn't find `zlayer-overlayd` next to `zlayer`. The `linux-amd64` and `linux-arm64` jobs now build `--package zlayer-overlayd` and bundle the binary in the tarball, and `install.sh` discovers it in the extracted archive and installs it alongside `zlayer` (rm-before-cp for the same macOS signature-cache reason as `zlayer-buildd`). (`.forgejo/workflows/build.yml`, `install.sh`)
...
### Added
- **Komodo end-to-end suite for the Docker shim.** Boots Komodo (mongo + core + periphery containers) with periphery's `DOCKER_HOST` pointed at the zlayer compat socket and drives the Komodo API end to end: Noise handshake to a healthy server, container listing, a real deployment, and container-log fetch asserted free of stdcopy frame bytes; plus direct shim checks for the JSON 404 fallback and a clean `/auth` credential rejection. New `komodo` suite in both e2e workflows; mongo added to the GHCR test-image mirror. (`crates/zlayer-docker/tests/komodo_e2e.rs`, `.forgejo/workflows/e2e.yml`, `.github/workflows/e2e.yml`, `mirror-test-images.yml`)
- **Dockerfile-compatibility e2e fixtures.** Buildah-backed builds of the constructs real Dockerfiles lean on — multi-stage + ARG-driven `RUN if/else` with staged artifact copy (the exact shape of the zbrain worker builds), `RUN --mount=type=cache`, pre-FROM ARG / stage-local ARG scoping with `COPY --from`, and ENV/WORKDIR/LABEL/EXPOSE/HEALTHCHECK/ENTRYPOINT metadata — each verified by reading marker files and image config back out of the built image. (`crates/zlayer-builder/tests/dockerfile_compat_e2e.rs`, `crates/zlayer-builder/tests/fixtures/dockerfiles/`)
- **macOS VZ-Linux guests now resync their wall clock after host sleep and on a periodic tick.** Apple Virtualization Linux guests have no RTC and never resynchronize after the host sleeps, so a long-lived VM's clock drifts (breaking x509 cert validity windows and any time-aware workload) even though the boot path already seeds it once via `zlayer.boottime=`. A new host-side `push_settime` sends `proto::Msg::SetTime { unix_secs }` (vzagent tag 14) over the vsock control channel, mirroring `push_overlay_agent` (fire-and-forget, warn on failure; a stale guest binary lacking tag 14 logs an unknown-tag error and drops the frame without crashing — the host just observes a reset). It is called best-effort after `VmLifecycleOp::Resume` in `unpause_container`, and a per-VM background task re-pushes it every 300s; the task's `JoinHandle` is stored on the container record and aborted on `stop`/`remove` (same lifetime pattern as the drain/cleanup tasks) so it exits cleanly when the VM stops. The boottime-injecting cmdline logic was factored into a unit-tested `build_linux_cmdline` helper. (`crates/zlayer-agent/src/runtimes/macos_vz_linux.rs`)
...
- **Data-dir `layout_version` marker with loud incompatibility detection.** The daemon writes `{data_dir}/layout_version` (currently 1), backfills it on legacy installs, refuses downgrades ("written by a newer zlayer") and corrupt markers, and runs a `detect_incompatible_stores` probe list before opening any store — a build that finds store formats it cannot read now fails startup naming the exact paths and remediation (`ZLAYER_DATA_DIR` or wipe) instead of silently starting empty beside them. (`bin/zlayer/src/migrations.rs`)
- **CI publishes the VZ-Linux guest bundle to GHCR** (`ghcr.io/blackleafdigital/zlayer/vz-linux:arm64` rolling + `:<kernel_version>-arm64` immutable) alongside the existing Forgejo generic-package publish, so the daemon's bundle cache has a normal image upstream. (`.forgejo/workflows/vz-linux-images.yml`)
- **The overlay DNS server now forwards non-overlay queries to upstream resolvers instead of refusing them.** Previously any query that didn't match the overlay zone got `REFUSED`, so a mesh VPN (netbird) registering a `~.` catch-all on systemd-resolved would break all host DNS and any container inheriting the host resolver broke with it. Each listener's catalog is now wrapped in a `ForwardingCatalog` that serves the overlay zone from the in-memory authority and forwards everything else to upstreams taken from `DnsConfig.upstreams` when set, otherwise auto-detected from `/etc/resolv.conf` with loopback / systemd-resolved-stub entries (`127.0.0.53` etc.) filtered out, falling back to `1.1.1.1`/`8.8.8.8` with a warning when nothing usable survives — so containers using the overlay DNS resolve public names independently of the host stub. Total upstream failure maps to `SERVFAIL` (bounded 2s/2-attempt resolver), never a silent empty `NoError`, and no new sockets are bound (no open recursion exposed). (`crates/zlayer-overlay/src/dns.rs`, `crates/zlayer-overlay/src/bootstrap.rs`)
- **Physical-NIC egress detection wired into the overlay so a mesh VPN can no longer hijack the advertised/source address.** Every "what is my source IP?" answer in the overlay stack used to come from the UDP-connect-to-`8.8.8.8` trick, which reports the address of whatever owns the *default route* — and on a mesh-joined node that route belongs to the VPN (netbird `wt*`, `WireGuard` `wg*`, Tailscale `utun*`, netbird `nb*`, …), so the overlay advertised and sourced through the tunnel instead of the real NIC. A new `detect_physical_egress()` resolver finds the physical egress interface and IP, skipping virtual/mesh devices (Linux reads the routing table over RTNETLINK in ascending-metric order, macOS uses `getifaddrs` plus the UDP-connect candidate, Windows keeps the UDP-connect trick verbatim). overlayd's `build_config` now binds WireGuard's `local_endpoint` to that physical IP — family-checked against the overlay IP, falling back to family-matched `UNSPECIFIED` on mismatch or detection failure — instead of always `UNSPECIFIED`; NAT host candidates prefer the physical-egress IP over the trick; and the NAT/STUN probe sockets are pinned to the NIC with `SO_BINDTODEVICE` (Linux) / `IP_BOUND_IF` (macOS), degrading to a warning (never a hard failure) on `EPERM`/`ENODEV`/unprivileged. boringtun 0.7 exposes no hook to pin its WireGuard *data* socket (`uapi_fd` is the UAPI control socket only), so source-IP selection via `local_endpoint` plus the advertised endpoint is documented in code as the realistic extent of egress control. (`crates/zlayer-overlay/src/egress.rs`, `crates/zlayer-overlayd/src/server.rs`, `crates/zlayer-overlay/src/nat/traversal.rs`, `crates/zlayer-overlay/src/nat/stun.rs`)
- **`zlayer serve` gains `--advertise-addr` (`ZLAYER_ADVERTISE_ADDR`) and `--dns-upstreams` (`ZLAYER_DNS_UPSTREAMS`).** Zero-config first-init advertised-address detection now resolves through `detect_physical_egress()` (physical NIC preferred over the default-route trick), and `--advertise-addr` overrides that resolution verbatim, persisting into `node_config.json`. `--dns-upstreams` pins the overlay DNS forwarder's non-overlay upstreams explicitly (comma-separated; bare IPs get `:53` appended; an unparseable entry is a hard startup error naming the bad entry) instead of auto-detecting from `/etc/resolv.conf`. Both flags ride process-global slots so the pinned `serve_with_external_shutdown` signature (Windows Service host) is left untouched. (`bin/zlayer/src/cli.rs`, `bin/zlayer/src/commands/serve.rs`, `bin/zlayer/src/daemon.rs`)
- **STUN server hostnames now resolve through the overlay's own upstream resolvers, not the (possibly hijacked) host resolver.** A literal `ip:port` (v4 or bracketed v6) short-circuits DNS entirely; a `host:port` is looked up through the same filtered-upstream bounded resolver the DNS forwarder uses (`build_forward_resolver` over stub-filtered, resolv.conf-detected upstreams), with `tokio::net::lookup_host` kept only as a warned last resort. This hardens NAT discovery against a netbird-style takeover that owns both the default route and the systemd-resolved `~.` catch-all. (`crates/zlayer-overlay/src/nat/stun.rs`)
- **TUN MTU is now set explicitly on the overlay interface (new `OverlayConfig.mtu`, default 1420).** The TUN MTU was previously never set on Linux/macOS, so WireGuard-in-WireGuard riding over a mesh silently blackholed oversized packets that couldn't be fragmented. The configured MTU is now applied at configure time on Linux (via RTNETLINK `set_link_mtu_by_name`) and macOS (via `ifconfig <name> mtu` after the kernel `utunN` name is discovered), and Windows replaces its hardcoded `1420` with the config value (Wintun fixes MTU at adapter-create time, so it's the only application point there). A failure to set the MTU degrades to a warning and continues with the kernel default rather than aborting overlay bring-up. (`crates/zlayer-overlay/src/config.rs`, `crates/zlayer-overlay/src/netlink.rs`, `crates/zlayer-overlay/src/interface.rs`, `crates/zlayer-overlay/src/transport.rs`)
- **Containers now get the overlay resolver injected instead of inheriting the host's `/etc/resolv.conf`.** The daemon binds a third overlay DNS listener on `<node_overlay_ip>:53` (resolv.conf `nameserver` and Docker `--dns` carry no port syntax — always 53 — so the existing overlay-port listener is unusable for native containers; binding port 53 needs root/`CAP_NET_BIND_SERVICE`, and on failure the daemon warns and proceeds, leaving containers on host resolv.conf — never fatal). When that listener binds, `ServiceManager` injects the node IP into `spec.dns` for non-host-network services that have no user-supplied DNS (an explicit spec resolver always wins), the Docker runtime carries it through unchanged via `HostConfig.dns`, and the youki bundle builder generates a minimal resolv.conf (`nameserver` lines plus `options edns0`, no inherited `search`/`domain`) and bind-mounts it read-only at `/etc/resolv.conf`. (`bin/zlayer/src/daemon.rs`, `crates/zlayer-agent/src/service.rs`, `crates/zlayer-agent/src/bundle.rs`)

### Changed
- **The macOS VZ-Linux guest kernel bundle is now a normally-pulled OCI image.** `ensure_linux_kernel` no longer requires an operator to hand-place `Image` + `initramfs.cpio.gz` in the cache: after the `ZLAYER_VZ_LINUX_KERNEL`/`_INITRD` dev override (still checked first, unchanged), the daemon pulls `ghcr.io/blackleafdigital/zlayer/vz-linux:arm64` through the SAME image machinery every other image uses (`ImagePuller` over the persistent `blobs.redb` blob cache, default hostname-based `AuthResolver` — anonymous GHCR once the package is public). The pulled manifest digest (read back under `manifest_digest_cache_key`) is compared against a `digest` file in the kernel cache; on a mismatch (or missing artifacts) the layer is unpacked and `vz-linux/Image` + `vz-linux/initramfs.cpio.gz` are atomically installed (write-`.tmp` + rename) with `manifest.json` copied alongside and the new digest recorded, while a matching digest reuses the cache as-is. On a pull failure (offline/401/registry down) a pre-existing cache is served with a warning; only when no cache exists does it error, now also naming the GHCR ref and that the package must be public. (`crates/zlayer-agent/src/runtimes/macos_vz_linux.rs`)
...
### Fixed
- **CI: artifact upload/download in `upload-temp-packages` and adjacent jobs has been flaking since v0.12.1.** Multiple root causes stacked: (1) `christopherhx/gitea-download-artifact@v4` parallel-downloads through `@actions/artifact`'s `streamExtractExternal`, which has a hardcoded 30 s per-request timeout that the largest artifact (226 MB `container-images`) trips on a self-hosted Forgejo whenever it has to share bandwidth with 9 sibling streams; (2) modern `@actions/artifact` (v2+) refuses to run against GHES/Forgejo at all (`GHESNotSupportedError`); (3) `scripts/check-existing-packages.sh` probed artifact presence with `curl -sI` (HEAD), but Forgejo's generic-packages router only registers GET and PUT, so every HEAD returned 405 and the script counted every artifact as missing forever, defeating the build-skip-when-already-published gate.
  
  Fixes:
  - New composite actions at `forge.blackleafdigital.com/Public/actions/{upload-artifact,download-artifact}@main` that talk directly to Forgejo's generic-packages API (`/api/packages/<owner>/generic/zlayer-artifacts/<run_id>/<name>.tar.gz`) via plain curl with retry — no `@actions/artifact` dependency, no GHES refusal, no per-request 30 s ceiling. Drop-in input compatibility with `actions/upload-artifact@v4` (adds a required `token` for the Forgejo auth pair).
  - Swapped every `christopherhx/gitea-upload-artifact@v4` (10 sites in `build.yml`, 6 in `publish-sdks.yml`) and every `download-artifact` site (4 in `build.yml`, 1 in `publish-sdks.yml`) to the new actions.
  - `scripts/check-existing-packages.sh` now uses 1-byte ranged GET (`curl --range 0-0`), matching the verify-artifact action — returns 206 for present, 404 for missing, never trips the 405. Re-dispatching `build.yml` for a fully-published version now correctly short-circuits with `need_build=false` and emits the "Build Skipped" summary.
  - The deprecated `Public/actions/download-artifacts@main` (sequential wrapper around `@actions/artifact`) was an intermediate step that hit fix #2 (GHES refusal) and has been superseded by the composite actions above. Left in place in the actions repo, no longer referenced from any workflow.
  (`.forgejo/workflows/build.yml`, `.forgejo/workflows/publish-sdks.yml`, `scripts/check-existing-packages.sh`, new actions in `Public/actions/upload-artifact/` and `Public/actions/download-artifact/`)
