...
- **2 of 3 Hyper-V e2e tests pass**: `windows_hcs_hyperv_smoke_create_start_stop_remove` ✅ and `windows_hcs_hyperv_concurrent_pair` ✅ (two independent UVMs in parallel). The `exec_inside_container` test fails because `exec()` uses the host `HcsCreateProcess` on the UVM (→ `0x80070032` ERROR_NOT_SUPPORTED — it can't create a process in the *hosted* container); exec must route over the GCS bridge (`RpcExecuteProcess` + hvsock stdio relay for stdout/stderr capture, like the init process) for Hyper-V isolation. Tracked for a follow-up.
