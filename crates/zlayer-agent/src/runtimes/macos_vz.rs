...
struct LiveVm {
    ...
    vm: Arc<QueuePinned<Retained<VZVirtualMachine>>>,
}
...
/// Resolve the entrypoint command from a [`ServiceSpec`] (run over SSH in the
/// guest). Falls back to a no-op `true` so a bare image still "runs".
fn resolve_entrypoint(spec: &ServiceSpec) -> Vec<String> {
...
/// Memory (MiB) requested by a spec, defaulting to 4096 when unset/unparseable
/// (a sane minimum for a macOS guest).
fn spec_memory_mib(spec: &ServiceSpec) -> u32 {
    spec.resources
        .memory
        .as_deref()
...
/// vCPUs requested by a spec, defaulting to 2.
#[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
fn spec_vcpus(spec: &ServiceSpec) -> u32 {
    spec.resources
        .cpu
        .map(|c| c.ceil() as u32)
        .filter(|&c| c > 0)
...
fn parse_dhcpd_lease_ip(contents: &str, mac: &str) -> Option<IpAddr> {
...
fn run_vm_lifecycle(live: &LiveVm, op: VmLifecycleOp) -> std::result::Result<(), String> {
...
fn read_vm_state(live: &LiveVm) -> VZVirtualMachineState {
...
impl Runtime for VzRuntime {
    ...
    async fn create_container(&self, id: &ContainerId, spec: &ServiceSpec) -> Result<()> {
        ...
        let vcpus = spec_vcpus(spec);
        let ram_mib = spec_memory_mib(spec);
...
// ---------------------------------------------------------------------------
// Filesystem / DHCP / SSH-key helpers
// ---------------------------------------------------------------------------

/// APFS `clonefile` copy-on-write from `src` to `dst`, falling back to a byte copy on any
/// failure (e.g. non-APFS volumes).
fn clone_or_copy(src: &Path, dst: &Path) -> std::io::Result<()> {
    if dst.exists() {
        return Ok(());
    }
...
async fn generate_ssh_keypair(key_path: &Path) {
    if key_path.exists() {
        return;
    }
...
async fn current_guest_ip(mac: &str) -> Option<IpAddr> {
