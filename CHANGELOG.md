...
### Known remaining (Windows Hyper-V containers — not yet working end-to-end)
- The external-GCS cold-start now runs all the way through: `NegotiateProtocol` / cold-start `Create`+`Start` / `configureHvSocketForGCS` / per-layer VSMB attach / SCSI scratch attach all succeed, and the guest now **accepts** the `CombineLayersWCOW` request (correct JSON shape). The remaining blocker is the **B4.3 guest-path round-trip**: `ContainerRootPath`/`ScratchPath` are still fabricated placeholder volume GUIDs (`Uuid::new_v4()`), so `CombineLayersWCOW` fails with `0x80070003` (path not found) — they must be the REAL in-guest mount paths returned by the guest's SCSI/VSMB mount responses. After that, the hosted-container `RpcCreate` config (`GuestOs.HostName` is rejected `$.HostName` — likely needs the hosted-system wrapper, not a bare `Container`) and the Hyper-V `start_container`/`stop`/`remove` lifecycle (host `HcsStartComputeSystem` on the already-running UVM returns `0x80370105`; the workload is started via `RpcStart`) need finishing. Tracked for a follow-up.
