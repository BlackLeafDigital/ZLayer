...
- **`ZLAYER_BUILDD_ADDR` env var** (read by the sidecar lifecycle but consumed from `SidecarConfig.addr`): dial a remote `zlayer-buildd` over TCP+mTLS for cross-machine build dispatch.
...
- **5 new RPCs** on the sidecar's `BuildService`: `Push`, `Tag`, `ManifestCreate`, `ManifestAdd`, `ManifestPush` — full parity with the BuildBackend trait surface so the sidecar handles every post-build operation the CLI shellout did.
...
- ZImagefile per-step `env` modifier on `run` steps. `crates/zlayer-builder/src/zimage/{parser,converter}.rs` now accept a `- run: ... + env: {K: V}` step shape; the env vars are emitted as transient `--env=K=V` flags on that single `buildah run` invocation and are NOT baked into the image's persistent ENV config (vs. `env` alone, which still emits a persistent `ENV` instruction). Combining `env` with `copy`/`add`/`workdir`/`user` is still rejected. Useful for build-time secrets like Forgejo / GitHub registry tokens that shouldn't persist in the resulting image. Implemented across `zimage/{parser,converter,types}.rs`, `dockerfile/instruction.rs::RunInstruction.env`, and `buildah/mod.rs` (`--env=K=V` emitted before the container ID, sorted alphabetically for determinism).
- `zlayer build --host-network` and `zlayer pipeline --host-network` now propagate to the buildah backend, emitting `--net=host` on every translated `buildah run`. Previously the flag existed on the top-level CLI but was a no-op for the builder (it only controlled the daemon's overlay-vs-host choice for runtime containers). Wired through `cli.host_network` → `commands::{build,pipeline}` → `ImageBuilder::with_host_network` → `BuildOptions.host_network` → `DockerfileTranslator::with_host_network`. Matches Docker's `docker build --network=host` semantic; also a workable escape hatch on hosts where buildah/netavark are version-skewed (see `docs/known-issues-linux-build.md`).

### Changed
- **Default Linux backend**: `detect_backend(ImageOs::Linux)` on Linux hosts now prefers `BuildahSidecarBackend` over the legacy CLI-shellout `BuildahBackend`. Falls back to CLI shellout when no sidecar binary is reachable. Operators wanting the legacy path explicitly should set `--backend buildah-cli` or `ZLAYER_BACKEND=buildah-cli`.
...
For Linux hosts that have buildah but not the new sidecar binary, no action is needed — the runtime detects the missing binary and falls back to CLI shellout transparently. Operators who want to opt in immediately:

```bash
cd bin/zlayer-buildd && make release-amd64
sudo install -m 0755 release/zlayer-buildd-linux-amd64 \
    /var/lib/zlayer/bin/zlayer-buildd
```

The path matches `zlayer_paths::ZLayerDirs::buildd_bin()` defaults; the discovery code finds it automatically on next build.
