...
//!
//! ```sh
//! cargo test -p zlayer-tunnel --test tunnel_overlay_e2e
//! ```
...
use zlayer_tunnel::client::OverlayAwareConnector;
use zlayer_tunnel::overlay::{
...
async fn test_node_tunnel_manager_with_overlay_resolver() {
    ...
    for status in manager.list_status() {
        assert_eq!(
            status.state,
            zlayer_tunnel::TunnelState::Disconnected,
