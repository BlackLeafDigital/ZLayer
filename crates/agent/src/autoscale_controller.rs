...
//!
//! ```ignore
//! use agent::autoscale_controller::AutoscaleController;
//! use agent::{ServiceManager, RuntimeConfig, create_runtime};
//! use std::sync::Arc;
...
use scheduler::metrics::{CgroupsMetricsSource, MetricsCollector, MetricsSource};
use scheduler::Autoscaler;
use spec::ScaleSpec;
        ...
        let cooldown = {
            ...
            match specs.get(service_name) {
                Some(ScaleSpec::Adaptive { cooldown, .. }) => {
                    cooldown.unwrap_or(scheduler::DEFAULT_COOLDOWN)
                }
...
pub fn has_adaptive_scaling(services: &HashMap<String, spec::ServiceSpec>) -> bool {
    services
        .values()
...
mod tests {
    ...
    use scheduler::metrics::{MockMetricsSource, ServiceMetrics};
    use spec::ScaleTargets;

    fn mock_spec() -> spec::ServiceSpec {
