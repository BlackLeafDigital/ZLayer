...
use builder::{list_templates, BuildEvent, ImageBuilder, Runtime, RuntimeInfo};
...
async fn run_build(
...
) -> std::result::Result<String, builder::BuildError> {
    ...
    if let Some(ref runtime_name) = request.runtime {
        let runtime = Runtime::from_name(runtime_name).ok_or_else(|| {
            builder::BuildError::InvalidInstruction {
