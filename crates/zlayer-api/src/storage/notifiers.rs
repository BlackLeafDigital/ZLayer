...
//! for notifiers (named notification channels that send alerts to Slack,
...
use super::sqlx_json::{JsonTable, SqlxJsonStore};
...
/// `SQLite`-backed persistent notifier store.
///
/// Delegates all storage operations to a [`SqlxJsonStore<StoredNotifier>`]
/// configured for the `notifiers` table. Records are serialised with
/// `serde_json` into the `data_json` column; the daemon reopens the database
/// on start-up and notifier configurations survive restarts.
pub struct SqlxNotifierStore {
    inner: SqlxJsonStore<StoredNotifier>,
}

impl SqlxNotifierStore {
    ...
    pub async fn open<P: AsRef<Path>>(path: P) -> Result<Self, StorageError> {
        let inner = SqlxJsonStore::open(path, NOTIFIERS_TABLE).await?;
        Ok(Self { inner })
    }
    ...
    pub async fn in_memory() -> Result<Self, StorageError> {
        let inner = SqlxJsonStore::in_memory(NOTIFIERS_TABLE).await?;
        Ok(Self { inner })
    }
}

#[async_trait]
impl NotifierStorage for SqlxNotifierStore {
    async fn store(&self, notifier: &StoredNotifier) -> Result<(), StorageError> {
...
mod tests {
    ...
    // =========================================================================
    // SqlxNotifierStore tests
    // =========================================================================

    #[tokio::test]
    async fn test_sqlx_store_and_get() {
        let store = SqlxNotifierStore::in_memory().await.unwrap();
        let n = make_notifier("slack-alerts", NotifierKind::Slack);
        let id = n.id.clone();
    ...
    #[tokio::test]
    async fn test_sqlx_delete_existing_and_missing() {
        let store = SqlxNotifierStore::in_memory().await.unwrap();
        let n = make_notifier("doomed", NotifierKind::Slack);
        let id = n.id.clone();
        store.store(&n).await.unwrap();
    ...
    #[tokio::test]
    async fn test_sqlx_upsert_same_id() {
    ...
    async fn test_sqlx_persistent_storage() {
        ...
        {
            let store = SqlxNotifierStore::open(&db_path).await.unwrap();
            store.store(&n).await.unwrap();
        }
