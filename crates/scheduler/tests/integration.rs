...
use scheduler::{
    Scheduler, SchedulerConfig, RaftConfig,
    ServiceMetrics, MockMetricsSource, ScalingDecision,
};
...
async fn test_standalone_scheduling_workflow() {
    ...
    mock.set_metrics("web-normal", vec![
        ServiceMetrics {
            cpu_percent: 50.0,
    ...
    mock.set_metrics("web-high-cpu", vec![
        ServiceMetrics {
            cpu_percent: 85.0,
            memory_bytes: 512 * 1024 * 1024,
            memory_limit: 1024 * 1024 * 1024,
...
async fn test_multiple_services() {
    ...
    for svc in ["api", "worker", "cache"] {
        ...
        assert!(!decision.is_change(), "Service {} should not scale at 50% CPU", svc);
    }
}
...
async fn test_prometheus_metrics() {
    ...
    mock.set_metrics("metrics-test", vec![
        ServiceMetrics {
            cpu_percent: 45.0,
            memory_bytes: 256 * 1024 * 1024,
            memory_limit: 512 * 1024 * 1024,
    ...
    assert!(!metrics.is_empty(), "Prometheus metrics should be registered");
...
async fn test_single_node_raft() {
    ...
    assert!(state.is_some(), "Cluster state should be available in distributed mode");
...
async fn test_scale_down_low_metrics() {
    ...
    mock.set_metrics("scale-down-test", vec![
        ServiceMetrics {
            cpu_percent: 20.0,
...
async fn test_memory_scaling() {
    ...
    mock.set_metrics("memory-test", vec![
        ServiceMetrics {
