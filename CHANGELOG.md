...
- With the above fixes the external-GCS cold-start now reaches step 6 and `NegotiateProtocol`/`Create`/`Start`/`configureHvSocketForGCS`/VSMB-attach all succeed. The remaining work is the **container storage subsystem for Hyper-V isolation**, which is built on process-isolated assumptions: the writable scratch is host-`ActivateLayer`'d (so SCSI-attach to the UVM hits `0x80070020` sharing violation — it must be created un-activated and formatted in-guest), and `CombineLayersWCOW` + the container root path still need the real per-VSMB/per-SCSI guest paths returned by in-guest mount responses (the step-7 `TODO`). These are tracked for a follow-up.
