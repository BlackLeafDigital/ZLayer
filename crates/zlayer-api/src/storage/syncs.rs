...
//! storage backends for sync resources — named records that point at a
//! directory within a project's git checkout containing `ZLayer` resource
//! YAMLs. Both backends share the [`SyncStorage`] trait so callers are
//! backend-agnostic.
//!
...
//! column; the in-memory backend enforces it in-process in [`create`] /
//! [`update`].
...
/// Two peeled columns:
///
/// - `name`: unique, single-column. Enforces global name uniqueness at the
///   schema level (so a second `create` with the same name surfaces as
///   [`StorageError::AlreadyExists`] from the underlying adapter).
/// - `project_id`: non-unique, nullable. Enables fast `project_id`-scoped
///   lookups via `SqlxJsonStore::get_by_unique("project_id", ...)`.
...
/// `SQLite`-backed persistent sync store.
///
/// Delegates all storage operations to a [`SqlxJsonStore<StoredSync>`]
/// configured for the `syncs` table. Records are serialised with `serde_json`
/// into the `data_json` column; the daemon reopens the database on start-up
/// and sync records survive restarts.
pub struct SqlxSyncStore {
    inner: SqlxJsonStore<StoredSync>,
}
...
impl SqlxSyncStore {
    ...
    pub async fn open<P: AsRef<Path>>(path: P) -> Result<Self, StorageError> {
        let inner = SqlxJsonStore::open(path, SYNCS_TABLE).await?;
        Ok(Self { inner })
    }
    ...
    pub async fn in_memory() -> Result<Self, StorageError> {
        let inner = SqlxJsonStore::in_memory(SYNCS_TABLE).await?;
        Ok(Self { inner })
    }
}

#[async_trait]
impl SyncStorage for SqlxSyncStore {
    async fn list(&self) -> Result<Vec<StoredSync>, StorageError> {
    ...
    async fn create(&self, sync_record: StoredSync) -> Result<StoredSync, StorageError> {
        // Preflight name-uniqueness check. The schema has a UNIQUE(name)
        // constraint so the adapter would still reject a conflict with
        // AlreadyExists, but checking ahead of time yields a cleaner error
        // message and avoids round-tripping the upsert machinery for a known
        // failure.
        if self
            .inner
            .get_by_unique("name", &sync_record.name)
            .await?
            .is_some()
        {
            return Err(StorageError::AlreadyExists(format!(
...
mod tests {
    ...
    #[tokio::test]
    async fn test_sqlx_list_and_get_roundtrip() {
        let store = SqlxSyncStore::in_memory().await.unwrap();
    ...
    #[tokio::test]
    async fn test_sqlx_create_duplicate_name() {
        let store = SqlxSyncStore::in_memory().await.unwrap();
    ...
    #[tokio::test]
    async fn test_sqlx_update_nonexistent() {
        let store = SqlxSyncStore::in_memory().await.unwrap();
    ...
    #[tokio::test]
    async fn test_sqlx_update_existing() {
        let store = SqlxSyncStore::in_memory().await.unwrap();
        let sync = StoredSync::new("updatable", "old/");
        let id = sync.id.clone();
        store.create(sync.clone()).await.unwrap();
    ...
    #[tokio::test]
    async fn test_sqlx_delete_missing_field_defaults_false() {
    ...
    #[tokio::test]
    async fn test_sqlx_delete_missing_field_persists_true() {
        let store = SqlxSyncStore::in_memory().await.unwrap();
    ...
    async fn test_sqlx_persistent_round_trip() {
        ...
        {
            let store = SqlxSyncStore::open(&db_path).await.unwrap();
            store.create(sync.clone()).await.unwrap();
        }
