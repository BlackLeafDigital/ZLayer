...
//! for plaintext key-value variables. Variables are NOT encrypted — they are
//! used for template substitution in deployment specs. Each variable has a
//! name that is unique within its scope (project id or global).
...
///
/// Note: `SQLite` treats `NULL`s as distinct in `UNIQUE`, so the compound
/// constraint only enforces uniqueness among variables where `scope` is
/// non-`NULL`. Global variables (`scope = None`) need application-level
/// enforcement; [`SqlxVariableStore::store`] does a pre-write `get_by_name`
...
/// `SQLite`-backed persistent variable store.
///
/// Delegates storage to a [`SqlxJsonStore<StoredVariable>`] configured for
/// the `variables` table. Records are serialised with `serde_json` into the
/// `data_json` column; the daemon reopens the database on start-up and
/// variable definitions survive restarts.
pub struct SqlxVariableStore {
    inner: SqlxJsonStore<StoredVariable>,
}

impl SqlxVariableStore {
    ...
    pub async fn open<P: AsRef<Path>>(path: P) -> Result<Self, StorageError> {
        let inner = SqlxJsonStore::open(path, VARIABLES_TABLE).await?;
        Ok(Self { inner })
    }
    ...
    pub async fn in_memory() -> Result<Self, StorageError> {
        let inner = SqlxJsonStore::in_memory(VARIABLES_TABLE).await?;
        Ok(Self { inner })
    }
}

#[async_trait]
impl VariableStorage for SqlxVariableStore {
    async fn store(&self, var: &StoredVariable) -> Result<(), StorageError> {
        // SQLite's UNIQUE(name, scope) only constrains rows where every
        // referenced column is non-NULL. Global variables (scope = None) slip
        // past the schema check, so we enforce (name, scope) uniqueness in
        // application code by looking up the existing row and rejecting when
        // the id differs.
        //
        // This is a TOCTOU between the read and write; it's acceptable
        // because the `SqlxJsonStore` uses a single-process SQLite pool with
        // WAL mode and a single writer, matching the in-memory store's
        // semantics. Under concurrent contention the schema-level UNIQUE
        // still enforces the invariant for scoped rows, and the window for
        // global rows is narrow enough to tolerate.
    ...
    async fn get_by_name(
        ...
        // The adapter's list_where_opt filters on the `scope` column using
        // SQL `IS NULL` when `scope` is `None`, so this branch handles both
        // global and scoped lookups without a separate code path.
...
mod tests {
    ...
    // =========================================================================
    // SqlxVariableStore tests
    // =========================================================================

    #[tokio::test]
    async fn test_sqlx_store_and_get() {
        let store = SqlxVariableStore::in_memory().await.unwrap();
        let var = make_var("APP_VERSION", "1.0.0", None);
        let id = var.id.clone();
    ...
    #[tokio::test]
    async fn test_sqlx_scope_isolation() {
        let store = SqlxVariableStore::in_memory().await.unwrap();
        ...
        // the NULL case that SQLite's UNIQUE treats as distinct.
    ...
    #[tokio::test]
    async fn test_sqlx_list_filtered_by_scope() {
        let store = SqlxVariableStore::in_memory().await.unwrap();
    ...
    #[tokio::test]
    async fn test_sqlx_delete_by_id() {
        let store = SqlxVariableStore::in_memory().await.unwrap();
        let var = make_var("DOOMED", "x", None);
        let id = var.id.clone();
        store.store(&var).await.unwrap();
    ...
    #[tokio::test]
    async fn test_sqlx_update_same_id() {
    ...
    async fn test_sqlx_persistent_round_trip() {
        ...
        {
            let store = SqlxVariableStore::open(&db_path).await.unwrap();
