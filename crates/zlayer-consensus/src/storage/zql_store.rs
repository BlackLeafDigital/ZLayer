...
impl<C: RaftTypeConfig<NodeId = NodeId>> ZqlLogStore<C> {
    ...
    /// # Errors
    ///
    /// Returns [`ConsensusError::Zql`](crate::error::ConsensusError::Zql) if the parent directory
    /// cannot be created or the ZQL database cannot be opened.
    ...
    C::Entry: serde::Serialize + serde::de::DeserializeOwned + Clone,
{
    ...
    async fn get_log_state(&mut self) -> Result<LogState<C>, StorageError<NodeId>> {
        ...
        // Read last_purged
        let last_purged: Option<LogId<NodeId>> = read_meta(&mut db, META_LAST_PURGED)?;
    ...
    async fn read_committed(&mut self) -> Result<Option<LogId<NodeId>>, StorageError<NodeId>> {
        let mut db = self.db.lock().await;
        read_meta(&mut db, META_COMMITTED)
    }
    ...
    F: Fn(&mut S, &C::D) -> C::R + Send + Sync + 'static,
{
    ...
    fn load_cache_blocking(db: &Arc<Mutex<zql::Database>>) -> ZqlSmCache<C, S> {
        ...
        let Ok(mut db) = db.try_lock() else {
