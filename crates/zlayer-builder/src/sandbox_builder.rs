...
///
/// Recognises `apt-get install`, `apt install`, `apk add`, `yum install`,
/// and `dnf install` invocations, including through `&&`-chained compound
/// commands and `sudo` prefixes.
///
/// # Examples
///
/// - `apt-get install -y curl git` -> `["curl", "git"]`
