...
async fn cleanup_stale_daemon(config: &DaemonConfig, socket_path: &str, api_bind: &str) {
    ...
    // -----------------------------------------------------------------------
    // 4b. Wait for WireGuard UDP port to be free
    // -----------------------------------------------------------------------
    {
...
pub(crate) async fn serve(
    ...
    //     the deployment ZqlStorage in daemon.rs which is tied to the S3
    //     ZqlReplicator path.
    // -----------------------------------------------------------------------
    ...
    let storage_api_state = zlayer_api::StorageState::new(replicator.clone());
    ...
    // Flush ZQL replicator before tearing down infrastructure
    if let Some(ref replicator) = replicator {
        if let Err(e) = replicator.flush().await {
            warn!("Failed to flush ZQL replicator: {e}");
        }
    }
...
///
/// [`DaemonState::storage`] (the deployment `ZqlStorage`) is still opened
/// inside `init_daemon` because the S3 `ZqlReplicator` is bound to its
/// exact `{data_dir}/deployments.db` path and the restore path depends on
/// the concrete `Arc<ZqlStorage>` type. Every *other* persistent store flows
/// through this bundle so serve.rs has a single source of truth.
///
/// When a `data_dir` is provided, each field is backed by a ZQL database at
/// `{data_dir}/<name>.db`, matching the one-directory-per-store layout used
/// for `deployments.db`. When `data_dir` is `None`, every store falls back
/// to an in-memory implementation — used by tests and any hypothetical
/// ephemeral mode.
...
impl StorageBundle {
    ...
    /// per-store in-memory implementations when `data_dir` is `None`.
    ///
    /// # Errors
    ///
    ...
    pub(crate) async fn open(data_dir: Option<&std::path::Path>) -> Result<Self> {
        if let Some(dir) = data_dir {
            ...
            let users = zlayer_api::ZqlUserStore::open(dir.join("users.db"))
                .await
                .context("Failed to open users.db")?;
            let environments = zlayer_api::ZqlEnvironmentStore::open(dir.join("environments.db"))
                .await
                .context("Failed to open environments.db")?;
            let projects = zlayer_api::ZqlProjectStore::open(dir.join("projects.db"))
                .await
                .context("Failed to open projects.db")?;
            let variables = zlayer_api::ZqlVariableStore::open(dir.join("variables.db"))
                .await
                .context("Failed to open variables.db")?;
            let tasks = zlayer_api::ZqlTaskStore::open(dir.join("tasks.db"))
                .await
                .context("Failed to open tasks.db")?;
            let workflows = zlayer_api::ZqlWorkflowStore::open(dir.join("workflows.db"))
                .await
                .context("Failed to open workflows.db")?;
            let notifiers = zlayer_api::ZqlNotifierStore::open(dir.join("notifiers.db"))
                .await
                .context("Failed to open notifiers.db")?;
            let groups = zlayer_api::ZqlGroupStore::open(dir.join("groups.db"))
                .await
                .context("Failed to open groups.db")?;
            let permissions = zlayer_api::ZqlPermissionStore::open(dir.join("permissions.db"))
                .await
                .context("Failed to open permissions.db")?;
            let audit = zlayer_api::ZqlAuditStore::open(dir.join("audit.db"))
                .await
                .context("Failed to open audit.db")?;
            let syncs = zlayer_api::ZqlSyncStore::open(dir.join("syncs.db"))
                .await
            ...
            Ok(Self {
                users: std::sync::Arc::new(zlayer_api::InMemoryUserStore::new()),
                environments: std::sync::Arc::new(zlayer_api::InMemoryEnvironmentStore::new()),
                projects: std::sync::Arc::new(zlayer_api::InMemoryProjectStore::new()),
                variables: std::sync::Arc::new(zlayer_api::InMemoryVariableStore::new()),
                tasks: std::sync::Arc::new(zlayer_api::InMemoryTaskStore::new()),
                workflows: std::sync::Arc::new(zlayer_api::InMemoryWorkflowStore::new()),
                notifiers: std::sync::Arc::new(zlayer_api::InMemoryNotifierStore::new()),
                groups: std::sync::Arc::new(zlayer_api::InMemoryGroupStore::new()),
                permissions: std::sync::Arc::new(zlayer_api::InMemoryPermissionStore::new()),
                audit: std::sync::Arc::new(zlayer_api::InMemoryAuditStore::new()),
                syncs: std::sync::Arc::new(zlayer_api::InMemorySyncStore::new()),
            })
        }
    }
}
