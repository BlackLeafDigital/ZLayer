...
- gcs: the cold-start `Create`'s `TimeZoneInformation` now matches hcsshim's `noInheritHostTimezone` UTC constant `utcTimezone` (`internal/uvm/timezone.go`) exactly — canonical names `"Coordinated Universal Time"` for both `StandardName`/`DaylightName` plus empty `StandardDate`/`DaylightDate` objects (`{}`), replacing the malformed bare `"UTC"` body that `TIME_ZONE_INFORMATION` rejects. Removes the malformed-TZ confounder from the 0xEF cold-start-death bisection.
