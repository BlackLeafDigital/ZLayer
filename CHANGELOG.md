...
- The smoke lifecycle passes. Still to verify/finish: the `exec` round-trip e2e (host `HcsCreateProcess` runs in the UVM, not the hosted container — `exec` must route over the GCS bridge for Hyper-V) and the concurrent-pair e2e. Tracked for a follow-up.
