...
use proxy::{ResolvedService, ServiceRegistry};
use spec::{DependsSpec, Protocol, ResourceType, ServiceSpec};
    ...
    pub async fn deploy_with_dependencies(
        ...
        for service_name in order {
            ...
            let replicas = match &service_spec.scale {
                ...
                spec::ScaleSpec::Adaptive { min, .. } => *min, // Start with min replicas
                spec::ScaleSpec::Manual => 1,                  // Default to 1 for manual scaling
            };
...
"#,
    ...
    fn dep(
        service: &str,
        condition: spec::DependencyCondition,
        timeout_ms: u64,
        on_timeout: spec::TimeoutAction,
    ) -> DependsSpec {
        DependsSpec {
            service: service.to_string(),
            condition,
    ...
    async fn test_deploy_with_dependencies_linear() {
        ...
        services.insert(
            ...
            mock_spec_with_deps(vec![dep(
                "c",
                spec::DependencyCondition::Started,
                5000,
                spec::TimeoutAction::Fail,
            )]),
        );
        services.insert(
            "a".to_string(),
    ...
    async fn test_deploy_with_dependencies_timeout_continue() {
        ...
        services.insert(
            ...
            mock_spec_with_deps(vec![dep(
                "b",
                spec::DependencyCondition::Healthy, // B won't pass healthy check
                100,                                // Short timeout
                spec::TimeoutAction::Continue,      // But continue anyway
            )]),
        );
    ...
    async fn test_deploy_with_dependencies_timeout_warn() {
        ...
        services.insert(
            ...
            mock_spec_with_deps(vec![dep(
                "b",
                spec::DependencyCondition::Healthy,
                100,
                spec::TimeoutAction::Warn,
            )]),
        );
    ...
    async fn test_check_dependencies_all_satisfied() {
        ...
        let deps = vec![DependsSpec {
            ...
            condition: spec::DependencyCondition::Healthy,
            timeout: Some(Duration::from_secs(60)),
            on_timeout: spec::TimeoutAction::Fail,
        }];
